...
for epub_file in "$dir_path"/*.epub; do
  if [ -f "$epub_file" ]; then
    ...
    # Perform modifications in the EPUB directory
    for filepath in "$epub_dir"/chapter_*.xhtml; do
      if [ -f "$filepath" ]; then
        awk 'BEGIN{RS="\0"; ORS=""} {sub("<div class=\"entry-content\" id=\"htmlContent\">","&<p>"); gsub("<br/><br/>","</p><p>"); match($0, /.*<\/div>/); pre=substr($0, RSTART, RLENGTH-6); post=substr($0, RSTART+RLENGTH-6); $0=pre "</p>" post; print}' "$filepath" > "${filepath}.tmp" && mv "${filepath}.tmp" "$filepath"
        perl -pi -e 's|<p>\s*|<p>|g' "$filepath"
        perl -pi -e 's|\s*</p>|</p>|g' "$filepath"
        perl -pi -e "s|<p>'([^<>]*?)'</p>|<p><i>\1</i></p>|g" "$filepath"
